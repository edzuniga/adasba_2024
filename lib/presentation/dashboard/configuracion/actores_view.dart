...
import 'package:adasba_2024/presentation/dashboard/data_table_sources/actores_dts.dart';
...
class ActoresView extends ConsumerStatefulWidget {
  const ActoresView({super.key});

  @override
  ConsumerState<ActoresView> createState() => _ActoresViewState();
}

class _ActoresViewState extends ConsumerState<ActoresView> {
  Timer? _timer;
  ...
  Widget build(BuildContext context) {
    final actores = ref.watch(actoresManagerProvider);
    return SafeArea(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Center(
              child: Text(
                'Tabla de Actores Participantes',
            ...
            Expanded(
              child: actores.when(
                data: (actores) {
                  //Lógica de filtrado
                  final filteredActores = _searchQuery.isEmpty
                      ? actores
                      : actores.where((actor) {
                          final nombre = actor.nombre.toLowerCase();
                          return nombre.contains(_searchQuery);
                        }).toList();
                  final actoresDTS = ActoresTableSource(
  ...
  Future<void> _addActorModal() async {
  ...
  Future<void> _generateDataRowsAndExcelArchive(List<Actor> actores) async {
    ...
    for (int i = 0; i < actores.length; i++) {
      rows.add([actores[i].nombre, actores[i].descripcion.toString()]);
    }
